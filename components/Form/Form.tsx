...
function Form({ ruLang, csrfToken }: Setting & { csrfToken: string }) {
  const {
    loader,
    formTexts,
    form,
    ...
    >
      {isSent ? (
        ...
        >
          ...
          <button onClick={reSubmit}>{formTexts.answerBtn}</button>
        </div>
      ) : (
        <form className={styles["form"]} onSubmit={formSubmit}>
          ...
          <button className={styles["submit"]} disabled={loader}>
            {formTexts.btn}
          </button>
        </form>
      )}
